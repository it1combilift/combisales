...
export function FormHeader({
  ...
  // Filter visible steps (exclude Step 3 if no containers, Step 4 if no special cargo)
  const visibleSteps = FORM_STEPS.filter((step) => {
    if (step.number === 3 && skipStep3) return false;
    if (step.number === 4 && skipStep4) return false;
    return true;
  });
